...
def request_token(
    ...
    if token_strategy == "exchange" and account is not None:
        try:
            ...
            acc = (
                account
                if isinstance(account, InternalAccount)
                else InternalAccount(account)
            )
...
def __load_oidc_configuration() -> bool:
    ...
    if not ADMIN_ISSUER_ID:
        logging.error(
            'Configuration option "admin_issuer" in section "oidc" is not set'
        )
        return False
    ...
    try:
        ...
        logging.error(
            'Failed to parse configuration file "%s"', IDPSECRETS, exc_info=True
        )
        return False
    try:
    ...
    try:
        ...
        if token_object:
            ...
            if not redirect_url:
                ...
                redirect_urls = [
                    u for u in client_secret["redirect_uris"] if redirect_to in u
                ]
    ...
    try:
        ...
        if not auto:
            ...
            auth_url = build_url(
                ...
                path="{}auth/oidc_redirect".format(
                    auth_server.path.split("auth/")[0].lstrip("/")
                ),
                params=access_msg,
            )
...
def get_token_oidc(
    ...
    try:
        ...
        oidc_client = __get_init_oidc_client(issuer=issuer, code=code, **client_params)[
            "client"
        ]
        ...
        if "scope" in oidc_tokens and "audience" in oidc_tokens:
            ...
            try:
                values = __get_keyvalues_from_claims(
                    oidc_tokens["access_token"], ["scope", "aud"]
                )
        ...
        if "refresh_token" in oidc_tokens:
            ...
            try:
                if oauth_req_params.refresh_lifetime is not None:
                    extra_dict["refresh_lifetime"] = int(
                        oauth_req_params.refresh_lifetime
                    )
            except Exception:
                pass
            try:
                values = __get_keyvalues_from_claims(
                    oidc_tokens["refresh_token"], ["exp"]
                )
                exp = values["exp"]
    ...
    try:
        ...
        if "access_token" in oidc_tokens:
            ...
            if validate_dict:
                ...
                logging.debug(
                    "Rucio could not get a valid admin token from the Identity Provider."
                )
                return None
        else:
            logging.debug(
                "Rucio could not get its admin access token from the Identity Provider."
            )
            return None
...
def __get_admin_account_for_issuer(*, session: "Session"):
    ...
    for issuer in OIDC_ADMIN_CLIENTS:
        admin_identity = oidc_identity_string(
            OIDC_ADMIN_CLIENTS[issuer].client_id, issuer
        )
    ...
    try:
        ...
        if admin and not get_token_for_adminacc:
            ...
            if not valid_subject_token_exists:
                logging.debug(
                    "No valid audience/scope exists for account %s token.", account
                )
                return None
            # openid scope is not supported for client_credentials auth flow - removing it if being asked for
            if "openid" in req_scope:
        ...
        logging.debug(
            "No token could be returned for account operation for account %s.", account
        )
        return None
    except Exception:
...
def __exchange_token_oidc(
    subject_token_object: models.Token, *, session: "Session", **kwargs
):
    """
    ...
    try:
        ...
        if "refresh_token" in oidc_tokens:
            extra_dict["refresh_token"] = oidc_tokens["refresh_token"]
            extra_dict["refresh"] = True
            extra_dict["refresh_lifetime"] = kwargs.get(
                "refresh_lifetime", REFRESH_LIFETIME_H
            )
            ...
            try:
                ...
                extra_dict["refresh_expired_at"] = datetime.utcfromtimestamp(
                    float(values["exp"])
                )
            except Exception:
                # 4 day expiry period by default
...
def refresh_cli_auth_token(
    ...
    if account_token.refresh:
        ...
        epoch_exp = int(
            floor((new_token["expires_at"] - datetime(1970, 1, 1)).total_seconds())
        )
        return new_token_string, epoch_exp
        ...
        if all_oidc_req_claims_present(
            ...
            epoch_exp = int(
                floor((new_token.expired_at - datetime(1970, 1, 1)).total_seconds())
            )
            new_token_string = new_token.token
            return new_token_string, epoch_exp
        # if scopes and audience are not the same, return None
        logging.debug(
            "No token could be returned for refresh operation for account %s.", account
        )
        return None
...
def __refresh_token_oidc(token_object: models.Token, *, session: "Session"):
    ...
    try:
        ...
        if datetime.utcnow() - extra_dict["refresh_start"] > timedelta(
            ...
            return None
        oidc_dict = __get_init_oidc_client(
            token_object=token_object, token_type="refresh_token"
        )  # noqa: S106
        oidc_client = oidc_dict["client"]
...
def delete_expired_oauthrequests(
    ...
    try:
        ...
        for states in session.execute(query).scalars().partitions(10):
            query = delete(models.OAuthRequest).where(
...
def __get_rucio_jwt_dict(jwt: str, account=None, *, session: "Session"):
    ...
    try:
        ...
        identity_string = oidc_identity_string(
            token_payload["sub"], token_payload["iss"]
        )
        ...
        if not account:
            ...
            account = get_default_account(
                identity_string, IdentityType.OIDC, True, session=session
            )
        else:
            if not exist_identity_account(
...
def validate_jwt(json_web_token: str, *, session: "Session") -> dict[str, Any]:
    ...
    try:
        ...
        token_dict: Optional[dict[str, Any]] = __get_rucio_jwt_dict(
            json_web_token, session=session
        )
        if not token_dict:
            raise CannotAuthenticate(traceback.format_exc())
...
@transactional_session
def save_subject_token(
    token, account, identity, scope, audience, lifetime, *, session: "Session"
):
