    ...
    def ensure_loaded(
        ...
        *,
        ...
        if self._attributes is None and load_attributes:
            self._attributes = list_rse_attributes(
                self.id, use_cache=True, session=session
            )
        ...
        if self._transfer_limits is None and load_transfer_limits:
            self._transfer_limits = get_rse_transfer_limits(
                rse_id=self.id, session=session
            )
    ...
    def bulk_load(
        ...
        *,
        ...
        session.bulk_insert_mappings(
            temp_table, ({"id": rse_id} for rse_id in rse_ids_to_load)
        )
        ...
        stmt = select(models.RSE).join_from(
            temp_table, models.RSE, models.RSE.id == temp_table.id
        )
        if not include_deleted:
            stmt = stmt.where(models.RSE.deleted == false())
        db_rses_by_id = {
            str(db_rse.id): db_rse for db_rse in session.execute(stmt).scalars()
        }

        if len(db_rses_by_id) != len(rse_ids_to_load):
            failed_rse_ids = ", ".join(rse_ids_to_load.difference(db_rses_by_id))
            raise exception.RSENotFound(
                f"RSE(s) with id(s) '{failed_rse_ids}' cannot be found"
            )
        ...
        if load_columns:
            ...
            for rse_id, db_rse in db_rses_by_id.items():
                ...
                columns = _format_get_rse(
                    db_rse=db_rse, rse_attributes=settings, session=session
                )
        ...
        if load_info:
            ...
            for rse_id, db_protocols in _group_query_result_by_rse_id(
        ...
        if load_limits:
            ...
            for rse_id, limits_list in _group_query_result_by_rse_id(
                stmt, session=session
            ):
                rse_id_to_data[rse_id]._limits = {
                    limit.name: limit.value for limit in limits_list
                }
        ...
        if load_usage:
            ...
            for rse_id, db_usages in _group_query_result_by_rse_id(
                stmt, session=session
            ):
...
class RseCollection(Generic[T]):
    ...
    def __init__(
        self, rse_ids: Optional["Iterable[str]"] = None, rse_data_cls: type[T] = RseData
    ):
...
def add_rse(
    ...
    *,
    ...
    availability = Availability(
        availability_read, availability_write, availability_delete
    ).integer
    new_rse = models.RSE(
        rse=rse,
        vo=vo,
    ...
    for setting in RSE_SETTINGS:
        ...
        if setting_value:
            add_rse_attribute(
                rse_id=new_rse.id, key=setting, value=setting_value, session=session
            )
...
def restore_rse(rse_id: str, *, session: "Session") -> None:
    ...
    try:
        stmt = select(models.RSE).where(
            and_(models.RSE.id == rse_id, models.RSE.deleted == true())
        )
        db_rse = session.execute(stmt).scalar_one()
    except sqlalchemy.orm.exc.NoResultFound:
        raise exception.RSENotFound("RSE with id '%s' cannot be found" % rse_id)
...
def _format_get_rse(
    ...
    *,
    ...
    if rse_attributes is not None:
        rse_settings = {
            key: rse_attributes[key] for key in RSE_SETTINGS if key in rse_attributes
        }
    else:
        ...
        rse_settings = {
            str(row.key): row.value for row in session.execute(stmt).scalars()
        }
...
def get_rse_id(
    ...
    try:
        stmt = select(models.RSE.id).where(
            and_(models.RSE.rse == rse, models.RSE.vo == vo)
        )
        if not include_deleted:
            stmt = stmt.where(models.RSE.deleted == false())
...
@read_session
def get_rse_name(
    rse_id: str, include_deleted: bool = True, *, session: "Session"
) -> str:
    """
        ...
        for k, v in filters.items():
            if hasattr(models.RSE, k):
                ...
                if isinstance(v, str):
                    try:
                        stmt = stmt.where(
                            getattr(models.RSE, k) == models.RSE.from_str(k, v)
                        )
...
def add_rse_attribute(
    ...
    try:
        ...
        raise exception.Duplicate(
            f"RSE attribute '{key}-{value}' for RSE '{rse}' already exists!"
        )
    return True
...
def parse_checksum_support_attribute(checksum_attribute: str) -> list[str]:
    ...
    supported_checksum_list = [
        c.strip() for c in checksum_attribute.split(",") if c.strip()
    ]
...
def set_rse_usage(
    ...
    *,
    ...
    """
    rse_usage = models.RSEUsage(
        rse_id=rse_id, source=source, used=used, free=free, files=files
    )
    ...
    for db_usage in db_usages:
        ...
        if per_account and db_usage.source == "rucio":
            ...
            for row in session.execute(stmt_account_usage).scalars():
                if row.bytes != 0:
                    percentage = (
                        round(float(row.bytes) / float(total) * 100, 2) if total else 0
                    )
...
def add_protocol(
    ...
    if "domains" in parameter:
        for domain in parameter["domains"]:
            ...
            for op in parameter["domains"][domain]:
                ...
                op_name = (
                    op
                    if op.startswith("third_party_copy")
                    else f"{op}_{domain}".lower()
                )
    ...
    try:
        ...
        if (
            ...
            or match(
                ".*IntegrityError.*1062.*Duplicate entry.*for key.*", error.args[0]
            )
            or match(
...
def _format_get_rse_protocols(
    ...
    *,
    ...
    if rse_attributes:
        ...
        verify_checksum = get_rse_attribute(
            _rse["id"], RseAttr.VERIFY_CHECKSUM, session=session
        )
...
def update_protocols(
    ...
    *,
    ...
    try:
        ...
        if up is None:
            msg = (
                "RSE '%s' does not support protocol '%s' for hostname '%s' on port '%s'"
...
def update_rse(rse_id: str, parameters: dict[str, Any], *, session: "Session"):
    ...
    if "rse" in param:
        add_rse_attribute(
            rse_id=rse_id, key=parameters["name"], value=True, session=session
        )
...
def fill_rse_expired(rse_id: str, *, session: "Session") -> None:
    ...
    models.RSEUsage(
        rse_id=rse_id, used=sum_bytes, files=sum_files, source="expired"
...
def determine_scope_for_rse(
    ...
    all_scopes = [f"{s}:{p}" for s in scopes for p in filtered_prefixes] + list(
        extra_scopes
    )
