...
| 6 | [Adding a new IdP / SCOPE_PROFILE](06-adding-a-new-idp-profile.md) | Checklist for validating the stack against a different external IdP — which files to add under `shared/config/<profile>/`, and the handful of places outside it (Helm's `configs-cm.yaml`, gitops' `seed-job.yaml`) that need a matching edit. |
