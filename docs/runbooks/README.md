...
| 6 | Certificate installation (Rucio + FTS) | Install the CA + host certs trusted by Rucio and FTS for IAM and storage backends, so token and TLS connections to external IdP/storage are trusted. Pending guidance from Andrea Manzi (CERN FTS/Rucio). | TODO |
