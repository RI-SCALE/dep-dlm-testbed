...
**EGI/LS AAI client registration is not self-service.** Replace
`<valid client id>`/`<valid client secret>` in
`shared/config/rucio/<profile>/idpsecrets.json`, then reseed:
```bash
...
`secrets.LS_AAI_DEV_OIDC_CLIENT_ID`/`_SECRET` (`.github/workflows/ls-aai-dev.yml`).
...
9. `client_id`/`client_secret` → `idpsecrets.json`.
...
6. `client_id`/`client_secret` → `shared/config/rucio/ls-aai-dev/idpsecrets.json`.
...
| Client id/secret | `idpsecrets.json` | per-environment |
...
2. Set issuer/audience/scopes per the table; ensure CA trust.
