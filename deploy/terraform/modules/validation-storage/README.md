...
| <a name="input_oidc_issuer_name"></a> [oidc\_issuer\_name](#input\_oidc\_issuer\_name) | Short label for this issuer, used as the YAML/section key in Teapot's application.yml and XRootD's scitokens.conf (e.g. 'keycloak-rucio', 'egi-checkin-dev', 'lsaai-dev'). No spaces — used as a bracketed section header in scitokens.conf. | `string` | n/a | yes |
...
| <a name="input_teapot_audiences"></a> [teapot\_audiences](#input\_teapot\_audiences) | Optional list of audience URIs Teapot restricts this issuer's tokens to (LS AAI profile only — EGI/Keycloak leave this empty). | `list(string)` | `[]` | no |
| <a name="input_teapot_extra_subs"></a> [teapot\_extra\_subs](#input\_teapot\_extra\_subs) | Additional 'sub' claims (beyond the two fixed teapot client UUIDs) mapped to the local 'teapot' user in user-mapping.csv — profile-specific client/human subs. | `list(string)` | `[]` | no |
| <a name="input_teapot_idp_name"></a> [teapot\_idp\_name](#input\_teapot\_idp\_name) | IdP\_name\_1 value in Teapot's STORAGE\_AREA\_1 config. | `string` | `"rucio"` | no |
